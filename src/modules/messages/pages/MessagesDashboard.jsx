...
import { 
  ...
  Sparkles, Sun, Moon, Phone, Mail, User,
  CheckCheck, Clock, MoreVertical, X
} from 'lucide-react'
...
export default function MessagesDashboard() {
  const { 
    ...
    markNotificationRead, markAllNotificationsRead, markAsRead
  } = useMessagesStore()
  ...
  return (
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">Communicate with your team</p>
          </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className="font-medium text-sm text-slate-800 dark:text-white truncate">
                            {conv.conversation_type === 'direct' ? otherUser?.profiles?.full_name || 'Unknown' : conv.subject || 'Group Chat'}
                          </p>
              ...
              <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-800 dark:text-white">
                        {otherParticipants[0]?.profiles?.full_name || 'Chat'}
                      </p>
                      <p className="text-xs text-slate-500">
                        {otherParticipants[0]?.profiles?.role?.replace('_', ' ') || ''}
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        <div className={`max-w-[70%] ${isMine ? 'order-1' : 'order-2'}`}>
                          ...
                          {msg.reactions?.length > 0 && (
                            <div className={`flex gap-1 mt-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                              {msg.reactions.map((r, i) => (
                                <span key={i} className="text-sm bg-white dark:bg-slate-600 px-1.5 py-0.5 rounded-full shadow-sm">{r.reaction}</span>
                              ))}
                            </div>
                          )}
                          {/* Hover Actions */}
                          {hoveredMessage === msg.id && !msg.is_deleted && (
                            <div className={`flex gap-1 mt-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                              ...
                              {isMine && (
                                <button onClick={() => deleteMessage(msg.id)} className="text-xs text-red-400 hover:text-red-600 ml-2">
                                  <X className="w-3 h-3" />
                                </button>
                              )}
                            </div>
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-700">
                  <div className="flex items-center gap-2">
                    <button className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400">
                      <Paperclip className="w-5 h-5" />
                    </button>
                    <input type="text" value={messageInput} onChange={(e) => setMessageInput(e.target.value)} onKeyPress={handleKeyPress}
                      placeholder="Type a message..." 
                      className="flex-1 p-3 neu-inset rounded-xl text-sm text-slate-700 dark:text-slate-300" />
                    <button onClick={handleSendMessage} disabled={!messageInput.trim()}
                      className="p-3 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {/* Notifications Panel */}
      {showNotifications && (
        <div className="fixed right-20 top-20 z-50 w-96 max-h-[70vh] overflow-y-auto neu-raised rounded-3xl bg-white dark:bg-slate-800 shadow-2xl" onClick={e => e.stopPropagation()}>
          <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><Bell className="w-4 h-4" />Notifications</h3>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {notifications.map(n => (
              <div key={n.id} onClick={() => { markNotificationRead(n.id); if (n.link_url) navigate(n.link_url); setShowNotifications(false) }}
