...
import { Send, Sparkles, Trash2, X, Clock, Zap, Bot } from 'lucide-react'
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-24 right-6 z-50 w-[400px] max-w-[95vw] h-[620px] max-h-[80vh] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden"
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-700 p-4 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button className="text-white/50 hover:text-white/80 p-1" title="Clear chat">
                <Trash2 className="w-4 h-4" />
              </button>
              <button onClick={toggleChat} className="text-white/70 hover:text-white p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                  ...
                  {msg.role === 'assistant' && (
                    <p className="text-[10px] font-bold text-purple-500 mb-1">KHUMO</p>
                  )}
