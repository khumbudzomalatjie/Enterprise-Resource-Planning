...
import { 
  ...
  Phone, FileText, Camera, Package, AlertCircle
} from 'lucide-react'
...
export default function OperationsDashboard() {
  ...
  // JOB TRACKER - Search by job number
  const handleTrackJob = async () => {
    ...
    try {
      ...
      // Get assigned employee info (who selected the job)
      let assignedInfo = null
      if (job.assigned_to) {
        const { data: emp } = await supabase
          .from('employees')
          .select('first_name, last_name, employee_code, phone')
      ...
      setTrackerError('Error loading job details')
    } finally {
      setTrackerLoading(false)
    }
  }
  ...
  const handleHoldJob = async (jobId, e) => {
    ...
    try {
      await supabase.from('jobs').update({ status: 'on_hold', updated_at: new Date().toISOString() }).eq('id', jobId)
      toast.success('Job put on hold ⏸️')
      loadData()
    } catch { toast.error('Failed to hold job') }
  }

  const handleResumeJob = async (jobId, e) => {
    e.stopPropagation()
    try {
      await supabase.from('jobs').update({ status: 'scheduled', updated_at: new Date().toISOString() }).eq('id', jobId)
      toast.success('Job resumed ▶️')
      loadData()
  ...
  const getPriorityColor = (priority) => {
    const colors = { low: 'text-slate-500', medium: 'text-blue-600', high: 'text-amber-600', urgent: 'text-red-600', emergency: 'text-red-600 animate-pulse' }
    return colors[priority] || 'text-slate-500'
  }

  const formatDate = (date) => date ? new Date(date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'
  const formatDateOnly = (date) => date ? new Date(date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'
  ...
  return (
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {jobCategories.map(cat => (
              <span key={cat.id} className="px-3 py-1.5 rounded-full text-xs font-medium"
                style={{ backgroundColor: cat.color + '20', color: cat.color, border: '1px solid ' + cat.color + '40' }}>{cat.name}</span>
            ))}
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-slate-200 dark:border-slate-700">
                <th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Job #</th><th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Client</th>
                <th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Category</th><th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Time</th>
                <th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Priority</th><th className="text-left text-sm font-medium text-slate-500 py-3 px-2">Status</th>
              ...
              <tbody>
                {todayJobs.map(job => (
                  <tr key={job.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    <td className="py-3 px-2 text-sm font-medium text-slate-800 dark:text-white cursor-pointer" onClick={() => navigate(`/operations/jobs/${job.id}`)}>{job.job_number}</td>
                    ...
                    {canManageJobs && (
                      <td className="py-3 px-2 text-center" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={(e) => handleOpenEdit(job, e)} className="px-2 py-1 rounded-lg bg-blue-100 text-blue-700 text-xs hover:bg-blue-200 flex items-center gap-1" title="Edit"><Edit3 className="w-3 h-3" /> Edit</button>
                          {job.status === 'on_hold' ? (
                            <button onClick={(e) => handleResumeJob(job.id, e)} className="px-2 py-1 rounded-lg bg-green-100 text-green-700 text-xs hover:bg-green-200 flex items-center gap-1"><Play className="w-3 h-3" /> Resume</button>
                          ) : job.status !== 'completed' && job.status !== 'cancelled' ? (
                            <>
                              <button onClick={(e) => handleHoldJob(job.id, e)} className="px-2 py-1 rounded-lg bg-purple-100 text-purple-700 text-xs hover:bg-purple-200 flex items-center gap-1"><Pause className="w-3 h-3" /> Hold</button>
                              <button onClick={(e) => handleCancelJob(job.id, e)} className="px-2 py-1 rounded-lg bg-red-100 text-red-700 text-xs hover:bg-red-200">✕</button>
                            </>
                          ) : null}
                        </div>
                      </td>
            ...
            {recentJobs.map(job => (
              <div key={job.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700/50 cursor-pointer" onClick={() => navigate(`/operations/jobs/${job.id}`)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canManageJobs && <button onClick={(e) => handleOpenEdit(job, e)} className="px-2 py-1 rounded-lg bg-blue-100 text-blue-700 text-xs hover:bg-blue-200"><Edit3 className="w-3 h-3" /></button>}
                </div>
              </div>
            ))}
          </div>
      ...
      <AnimatePresence>
        {showTracker && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            onClick={() => { setShowTracker(false); setTrackedJob(null); setTrackerError(null) }}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              {/* Tracker Header */}
              <div className="sticky top-0 bg-white dark:bg-slate-800 z-10 p-5 border-b border-slate-200 dark:border-slate-700">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                    <Search className="w-5 h-5 text-blue-600" /> Job Tracker
                  </h3>
                  <button onClick={() => { setShowTracker(false); setTrackedJob(null); setTrackerError(null) }}
                    className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700"><X className="w-5 h-5" /></button>
                </div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input type="text" value={trackerJobNumber} onChange={e => setTrackerJobNumber(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && handleTrackJob()}
                      placeholder="Enter Job Number (e.g., JOB-2506-0001)..."
                      className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm" />
                  </div>
                  <button onClick={handleTrackJob} disabled={trackerLoading}
                    ...
                    {trackerLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <Search className="w-4 h-4" />}
                    Track
                  </button>
                </div>
              </div>
              ...
              <div className="p-5">
                {trackerError && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-red-500 font-medium">{trackerError}</p>
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-5 border border-blue-200 dark:border-blue-800">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h2 className="text-xl font-bold text-slate-800 dark:text-white">{trackedJob.title}</h2>
                          <p className="text-sm text-slate-500">{trackedJob.job_number}</p>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(trackedJob.status)}`}>
                          {trackedJob.status?.replace('_', ' ')}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                        ...
                        <div><span className="text-slate-500">Category:</span> <span className="font-semibold" style={{ color: trackedJob.job_categories?.color }}>{trackedJob.job_categories?.name || 'N/A'}</span></div>
                      </div>
                      <div className="mt-2 text-sm"><MapPin className="w-3.5 h-3.5 inline text-slate-400 mr-1" />{trackedJob.site_address}</div>
                    </div>

                    {/* Timeline */}
                    <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                      <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><Timer className="w-4 h-4 text-blue-600" />Job Timeline</h3>
                      <div className="space-y-2">
                        <TimelineItem icon="📝" label="Created" value={trackedJob.creator?.full_name || 'System'} date={formatDate(trackedJob.created_at)} />
                        {trackedJob.notes?.includes('SELECTED BY:') && (
                          <TimelineItem icon="✋" label="Selected By" value={trackedJob.notes.split('at')[0].replace('SELECTED BY:', '').trim()} date={trackedJob.notes.split('at')[1]?.trim()} />
                        )}
                        {trackedJob.assigned_employee && (
                          <TimelineItem icon="👤" label="Assigned To" value={`${trackedJob.assigned_employee.first_name} ${trackedJob.assigned_employee.last_name}`} date={trackedJob.assigned_employee.employee_code} />
                        )}
                        {trackedJob.actual_start_time && (
                          <TimelineItem icon="🚀" label="Started" value={formatDate(trackedJob.actual_start_time)} />
                        )}
                        {trackedJob.actual_end_time && (
                          <TimelineItem icon="✅" label="Completed" value={formatDate(trackedJob.actual_end_time)} />
                        )}
                      </div>
                    </div>
                    ...
                    {trackedJob.clients && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><Building2 className="w-4 h-4 text-blue-600" />Client</h3>
                        <div className="space-y-1 text-sm">
                          <p className="font-semibold">{trackedJob.clients.company_name}</p>
                          {trackedJob.clients.phone && <p><Phone className="w-3 h-3 inline mr-1" />{trackedJob.clients.phone}</p>}
                          {trackedJob.clients.email && <p className="text-slate-500">{trackedJob.clients.email}</p>}
                          <p className="text-slate-500">{[trackedJob.clients.address_line1, trackedJob.clients.city].filter(Boolean).join(', ')}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {trackedJob.photos?.length > 0 && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><Camera className="w-4 h-4 text-blue-600" />Photos ({trackedJob.photos.length})</h3>
                        <div className="grid grid-cols-3 gap-2">
                          {trackedJob.photos.map(photo => (
                            <a key={photo.id} href={photo.photo_url} target="_blank" rel="noopener noreferrer" className="rounded-xl overflow-hidden">
                              <img src={photo.photo_url} alt={photo.caption || 'Job photo'} className="w-full h-24 object-cover" />
                              <span className="text-[9px] text-slate-500 block text-center capitalize">{photo.photo_type}</span>
                            </a>
                          ))}
                        </div>
                      </div>
                    ...
                    {trackedJob.tasks?.length > 0 && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-blue-600" />Tasks ({trackedJob.tasks.filter(t => t.is_completed).length}/{trackedJob.tasks.length})</h3>
                        <div className="space-y-1">
                          {trackedJob.tasks.map(task => (
                            <div key={task.id} className="flex items-center gap-2 text-sm">
                              <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${task.is_completed ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300'}`}>
                                {task.is_completed && <CheckCircle2 className="w-3 h-3 text-white" />}
                              </div>
                              <span className={task.is_completed ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-300'}>{task.description}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {trackedJob.signature && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-blue-600" />Client Signature</h3>
                        ...
                        <p className="text-xs text-slate-500">Rating: {trackedJob.signature.satisfaction_rating}/5</p>
                        {trackedJob.signature.signature_url && (
                          <img src={trackedJob.signature.signature_url} alt="Signature" className="mt-2 max-h-20 border rounded-lg" />
                        )}
                      </div>
                    )}
                    ...
                    {trackedJob.incidents?.length > 0 && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><AlertCircle className="w-4 h-4 text-red-600" />Incidents ({trackedJob.incidents.length})</h3>
                    ...
                    {trackedJob.invoice && (
                      <div className="bg-white dark:bg-slate-700 rounded-2xl p-5 border border-slate-200 dark:border-slate-600">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-emerald-600" />Invoice</h3>
                ...
                {!trackedJob && !trackerError && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-400 text-sm mt-1">All job details will appear here</p>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {editingJob && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setEditingJob(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="flex justify-between items-center p-5 border-b sticky top-0 bg-white dark:bg-slate-800">
                <div><h3 className="text-lg font-bold">Edit Job</h3><p className="text-xs text-slate-500">{editingJob.job_number}</p></div>
                <button onClick={() => setEditingJob(null)} className="p-2 rounded-xl hover:bg-slate-100"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-5 space-y-4">
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Title</label><input type="text" value={editForm.title} onChange={e => setEditForm({...editForm, title: e.target.value})} className="w-full p-3 rounded-xl border text-sm" /></div>
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Status</label><select value={editForm.status} onChange={e => setEditForm({...editForm, status: e.target.value})} className="w-full p-3 rounded-xl border text-sm"><option value="pending">Pending</option><option value="scheduled">Scheduled</option><option value="in_progress">In Progress</option><option value="completed">Completed</option><option value="on_hold">On Hold</option><option value="cancelled">Cancelled</option></select></div>
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Priority</label><select value={editForm.priority} onChange={e => setEditForm({...editForm, priority: e.target.value})} className="w-full p-3 rounded-xl border text-sm"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></div>
                <div className="grid grid-cols-3 gap-3">
                  <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Date</label><input type="date" value={editForm.scheduled_date} onChange={e => setEditForm({...editForm, scheduled_date: e.target.value})} className="w-full p-3 rounded-xl border text-sm" /></div>
                  <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Start</label><input type="time" value={editForm.scheduled_start_time} onChange={e => setEditForm({...editForm, scheduled_start_time: e.target.value})} className="w-full p-3 rounded-xl border text-sm" /></div>
                  <div><label className="text-xs font-semibold text-slate-500 mb-1 block">End</label><input type="time" value={editForm.scheduled_end_time} onChange={e => setEditForm({...editForm, scheduled_end_time: e.target.value})} className="w-full p-3 rounded-xl border text-sm" /></div>
                </div>
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Address</label><input type="text" value={editForm.site_address} onChange={e => setEditForm({...editForm, site_address: e.target.value})} className="w-full p-3 rounded-xl border text-sm" /></div>
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Notes</label><textarea value={editForm.notes} onChange={e => setEditForm({...editForm, notes: e.target.value})} rows={3} className="w-full p-3 rounded-xl border text-sm resize-none" /></div>
              </div>
              <div className="flex justify-end gap-3 p-5 border-t">
                <button onClick={() => setEditingJob(null)} className="px-5 py-2.5 rounded-xl bg-slate-200 text-sm">Cancel</button>
                <button onClick={handleSaveEdit} disabled={savingEdit} className="px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold flex items-center gap-2 disabled:opacity-50">
                  {savingEdit ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <Save className="w-4 h-4" />} Save
                </button>
              </div>
            </motion.div>
          </motion.div>
...
function TimelineItem({ icon, label, value, date }) {
  return (
