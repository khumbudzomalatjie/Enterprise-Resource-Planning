...
export const fieldOpsApi = {
  ...
  async getLiveJobs() {
    ...
    // ✅ Only show jobs that are NOT completed or cancelled
    ...
    const clientIds = [...new Set(openJobs.map(j => j.client_id).filter(Boolean))]
    const { data: clients } = await supabase
      .from('clients')
      .select('id, company_name, client_code, phone, city, address_line1')
      .in('id', clientIds)

    const catIds = [...new Set(openJobs.map(j => j.job_category_id).filter(Boolean))]
    const { data: categories } = await supabase
      .from('job_categories')
      .select('id, name, color')
      .in('id', catIds)

    const teamIds = [...new Set(openJobs.map(j => j.team_id).filter(Boolean))]
  ...
  async getJobAuditTrail(searchInput) {
    ...
    if (!job) {
      const { data: partial } = await supabase.from('jobs').select('*').ilike('job_number', `%${search}%`).limit(1).single()
      job = partial
    }
    ...
    let creator = null
    if (job.created_by) {
      const { data: c } = await supabase.from('profiles').select('full_name, email, role').eq('id', job.created_by).single()
      creator = c
    }

    return {
      data: {
        job: { ...job, clients: client?.data || null, job_categories: category?.data || null },
        creator,
