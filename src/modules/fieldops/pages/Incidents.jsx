...
import { 
  AlertTriangle, Plus, Search, Eye, ChevronRight,
  Sun, Moon, Sparkles
} from 'lucide-react'
...
export default function Incidents() {
  ...
  const filteredIncidents = incidents.filter(inc => {
    ...
    return inc.title?.toLowerCase().includes(s) ||
           inc.incident_number?.toLowerCase().includes(s) ||
           inc.employees?.first_name?.toLowerCase().includes(s)
  })

  const getSeverityColor = (severity) => {
    const colors = { low: 'bg-slate-100 text-slate-700', medium: 'bg-blue-100 text-blue-700', high: 'bg-amber-100 text-amber-700', critical: 'bg-red-100 text-red-700', fatal: 'bg-red-200 text-red-800' }
    return colors[severity] || 'bg-slate-100'
  }

  const getStatusColor = (status) => {
    const colors = { reported: 'bg-amber-100 text-amber-700', under_investigation: 'bg-blue-100 text-blue-700', resolved: 'bg-emerald-100 text-emerald-700', closed: 'bg-slate-100 text-slate-600', escalated: 'bg-purple-100 text-purple-700' }
    return colors[status] || 'bg-slate-100'
  }

  const getTypeIcon = (type) => {
    const icons = { accident: '💥', injury: '🏥', property_damage: '🏚️', equipment_failure: '🔧', chemical_spill: '🧪', near_miss: '⚠️', security: '🔒', customer_complaint: '📢', vehicle_accident: '🚗', weather: '🌩️', theft: '🔫', other: '📋' }
    return icons[type] || '📋'
  }
    ...
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 dark:text-white flex items-center gap-3"><AlertTriangle className="w-8 h-8 text-red-600" />Incidents</h1>
        ...
        <div className="neu-raised rounded-2xl p-4 mb-6 flex flex-col sm:flex-row gap-4">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-3 neu-inset rounded-xl text-slate-700 dark:text-slate-300">
            <option value="all">All Status</option><option value="reported">Reported</option><option value="under_investigation">Under Investigation</option><option value="resolved">Resolved</option><option value="closed">Closed</option>
          </select>
          <select value={severityFilter} onChange={(e) => setSeverityFilter(e.target.value)} className="px-4 py-3 neu-inset rounded-xl text-slate-700 dark:text-slate-300">
            <option value="all">All Severity</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option><option value="fatal">Fatal</option>
          </select>
        </div>
