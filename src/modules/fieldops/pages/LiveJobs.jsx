...
export default function LiveJobs() {
  ...
  useEffect(() => {
    console.log('🚀 LiveJobs mounted - loading data & subscribing to realtime')
    ...
    const jobsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔄 [REALTIME] Job changed:', payload.eventType, payload.new?.job_number || payload.old?.job_number)
          loadAllData()
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'field_job_assignments' },
        (payload) => {
          console.log('🔄 [REALTIME] Assignment changed:', payload.eventType)
          loadAllData()
          setLastSync(new Date())
        }
      )
      .subscribe((status) => {
        ...
        if (status === 'CHANNEL_ERROR') {
          console.error('❌ Realtime connection error - falling back to polling')
        }
      })
  ...
  const loadAllData = async () => {
    try {
      console.log('📥 Loading live jobs data...')
  ...
  const unifiedJobs = useCallback(() => {
    ...
    console.log(`📊 Unified ${merged.length} jobs (${merged.filter(j => j.isMyJob).length} mine)`)
    return merged
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        {!dataLoaded ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-slate-400 mt-1">Connecting to realtime updates...</p>
          </div>
        ) : sortedJobs.length === 0 ? (
              ...
              <motion.div key={job.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} layout
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {job.isMyJob && <span className="px-2 py-0.5 rounded-full text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">🔒 My Job</span>}
                        {job.job_categories && <span className="px-2 py-0.5 rounded-full text-xs" style={{ backgroundColor: job.job_categories.color + '20', color: job.job_categories.color }}>{job.job_categories.name}</span>}
                      </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {job.status === 'scheduled' && <button onClick={() => handleStartJob(job.id)} className="p-2 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200" title="Start"><Play className="w-4 h-4" /></button>}
                    ...
                    <button onClick={() => { setSelectedJob(job); setShowAssignModal(true) }} className="p-2 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200" title="Assign"><UserPlus className="w-4 h-4" /></button>
                ...
                <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold text-slate-500 flex items-center gap-2"><Users className="w-4 h-4" />
                      Staff ({job.field_job_assignments?.filter(a => a.assignment_status !== 'released' && a.assignment_status !== 'completed').length || 0}/{job.cleaners_required || 1})
                    </h4>
                    {(job.field_job_assignments?.filter(a => a.assignment_status !== 'released' && a.assignment_status !== 'completed').length || 0) >= (job.cleaners_required || 1) && (
                      <span className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Fully Staffed</span>
                    )}
                  </div>
                  {job.field_job_assignments?.filter(a => a.assignment_status !== 'released' && a.assignment_status !== 'completed').length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {job.field_job_assignments.filter(a => a.assignment_status !== 'released' && a.assignment_status !== 'completed').map(a => (
                        <div key={a.id} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm ${a.employees?.user_id === user?.id ? 'bg-emerald-100 text-emerald-700 ring-2 ring-emerald-500' : 'bg-slate-100 text-slate-700'}`}>
                          <div className="w-6 h-6 rounded-full bg-slate-300 flex items-center justify-center text-xs font-bold">{a.employees?.first_name?.[0]}{a.employees?.last_name?.[0]}</div>
                          ...
                          <button onClick={() => handleRelease(a.id, `${a.employees?.first_name} ${a.employees?.last_name}`, job.job_number)} className="ml-1 p-0.5 rounded-full hover:bg-red-200 text-slate-400 hover:text-red-600" title="Release"><XCircle className="w-3.5 h-3.5" /></button>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-sm text-slate-400 italic">No staff assigned - Click <UserPlus className="w-3 h-3 inline" /> to assign</p>
                  )}
                </div>
        ...
        {showAssignModal && selectedJob && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAssignModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="neu-raised rounded-3xl p-6 max-w-md w-full bg-white dark:bg-slate-800" onClick={e => e.stopPropagation()}>
              ...
              <select value={selectedEmployee} onChange={(e) => setSelectedEmployee(e.target.value)} className="w-full p-3 neu-inset rounded-xl mb-4">
                <option value="">Select Employee</option>
                {availableEmployees.map(emp => (
                  <option key={emp.id} value={emp.id}>{emp.first_name} {emp.last_name} ({emp.employee_code}) - {emp.department}</option>
                ))}
              </select>
