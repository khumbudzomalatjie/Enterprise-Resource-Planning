...
export default function FieldOpsDashboard() {
  ...
  const statCards = [
    ...
    { icon: MessageSquare, label: 'Messages', value: 'Chat', color: 'text-teal-600', bg: 'bg-teal-100 dark:bg-teal-900/30', path: '/fieldops/messages' },
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            ...
            <motion.div key={card.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }}
              ...
              className="neu-raised rounded-3xl p-6 cursor-pointer hover:scale-[1.02] transition-transform text-center">
              <div className={`w-16 h-16 rounded-2xl ${card.bg} flex items-center justify-center mx-auto mb-3`}>
                <card.icon className={`w-8 h-8 ${card.color}`} />
              </div>
              <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-2">{card.label}</h2>
              <p className="text-sm text-slate-500 mb-3">{card.desc}</p>
              <span className="text-xs px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400">{card.stats}</span>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          {[
            ...
            { icon: FileText, label: 'Report Incident', path: '/fieldops/incidents/report', color: 'text-amber-600', bg: 'bg-amber-100 dark:bg-amber-900/30' },
          ].map(action => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {statCards.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.05 }}
              ...
              className={`neu-raised rounded-2xl p-4 ${stat.path ? 'cursor-pointer stat-card hover:scale-105' : ''}`}>
