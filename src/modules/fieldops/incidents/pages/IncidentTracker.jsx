...
import { 
  ...
  CheckCircle2, XCircle, FileText, Eye, MessageSquare,
  Camera, MapPin, Phone, Mail, Building2, Wrench,
  Car, Heart, ArrowRight, ChevronRight, ArrowLeft,
  Sun, Moon, Sparkles, Activity, Users, Calendar,
  ClipboardCheck, Send, Download, Printer
} from 'lucide-react'

export default function IncidentTracker() {
  const { fetchIncident, selectedIncident, loading } = useIncidentStore()
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="neu-raised rounded-3xl p-6 border-l-4 border-purple-500">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>{formatDate(trackerData.incident.incident_date)} at {trackerData.incident.incident_time?.slice(0,5)}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <MapPin className="w-4 h-4 text-slate-400" />
                  <span>{trackerData.incident.site || 'N/A'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Building2 className="w-4 h-4 text-slate-400" />
                  <span className="capitalize">{trackerData.incident.incident_category?.replace(/_/g, ' ')}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Clock className="w-4 h-4 text-slate-400" />
                  <span>SLA: {formatDate(trackerData.incident.sla_due_date)}</span>
                </div>
              </div>
            </div>

            {/* WHO REPORTED + WHO INVESTIGATED */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Reporter */}
              <div className="neu-raised rounded-3xl p-6">
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                  <User className="w-5 h-5 text-blue-600" />Reported By
                </h3>
                <div className="space-y-2 text-sm">
                  ...
                  {trackerData.reporter?.email && (
                    <p className="flex items-center gap-1 text-slate-500"><Mail className="w-3 h-3" />{trackerData.reporter.email}</p>
                  )}
                  <p className="text-xs text-slate-400">Reported: {formatDateTime(trackerData.incident.reported_at)}</p>
                  {trackerData.incident.reporting_device && (
                    <p className="text-xs text-slate-400 truncate">Device: {trackerData.incident.reporting_device?.substring(0, 50)}</p>
                  )}
                </div>
              </div>
              ...
              <div className="neu-raised rounded-3xl p-6">
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                  <Search className="w-5 h-5 text-purple-600" />Investigator
                </h3>
                {trackerData.investigator ? (
                  <div className="space-y-2 text-sm">
                    ...
                    <p className="flex items-center gap-1 text-slate-500"><Mail className="w-3 h-3" />{trackerData.investigator.email}</p>
                    {trackerData.incident.investigation_started_at && (
                      <p className="text-xs text-slate-400">Started: {formatDateTime(trackerData.incident.investigation_started_at)}</p>
                    )}
                    {trackerData.incident.investigation_completed_at && (
                      <p className="text-xs text-emerald-600">Completed: {formatDateTime(trackerData.incident.investigation_completed_at)}</p>
                    )}
                  </div>
                ) : (
              ...
              <div className="neu-raised rounded-3xl p-6">
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                  <Briefcase className="w-5 h-5 text-emerald-600" />Related
                </h3>
                <div className="space-y-2 text-sm">
                  {trackerData.job && (
                    <div>
                      <p className="text-xs text-slate-500">Job</p>
                      <p className="font-medium">{trackerData.job.job_number} - {trackerData.job.title}</p>
                    </div>
                  )}
                  {trackerData.client && (
                    <div>
                      <p className="text-xs text-slate-500">Client</p>
                      <p className="font-medium">{trackerData.client.company_name}</p>
                      {trackerData.client.phone && <p className="flex items-center gap-1 text-slate-500"><Phone className="w-3 h-3" />{trackerData.client.phone}</p>}
                    </div>
                  )}
                  {!trackerData.job && !trackerData.client && (
                    <p className="text-sm text-slate-400 italic">No job or client linked</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="neu-raised rounded-3xl p-6">
                ...
                <div className="space-y-2 text-sm">
                  <div><span className="text-slate-500">People Involved:</span> {trackerData.incident.people_involved || 'None'}</div>
                  ...
                  <div className="flex items-center gap-2">{trackerData.incident.medical_treatment ? '✅' : '❌'} Medical Treatment</div>
              ...
              {trackerData.auditLog.length > 0 ? (
                <div className="relative">
                  ...
                  <div className="space-y-4">
                    {trackerData.auditLog.map((log, i) => (
                      <div key={log.id} className="relative pl-12">
                        ...
                        <div className="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                          ...
                          <div className="flex items-center gap-3 p-2 rounded-lg bg-slate-50 dark:bg-slate-700/30">
                            ...
                            <div>
                              <p className="text-sm font-medium text-slate-800 dark:text-white">{log.performed_by_name || 'System'}</p>
                              <p className="text-xs text-slate-500 capitalize">{log.performed_by_role?.replace(/_/g, ' ') || 'N/A'}</p>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {trackerData.correctiveActions.length > 0 && (
              <div className="neu-raised rounded-3xl p-6">
                <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                  <Wrench className="w-5 h-5 text-orange-600" />Corrective & Preventive Actions ({trackerData.correctiveActions.length})
                </h2>
                  ...
                  {trackerData.correctiveActions.map(action => (
                    <div key={action.id} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm text-slate-800 dark:text-white">{action.title}</p>
                          <p className="text-xs text-slate-500 capitalize">{action.action_type} • {action.priority} • Due: {formatDate(action.due_date)}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className={`px-2 py-0.5 rounded-full text-xs ${action.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : action.status === 'in_progress' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                          {action.status}
                        </span>
                        {action.progress_percentage > 0 && (
                          <p className="text-xs text-slate-500 mt-1">{action.progress_percentage}% complete</p>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <div className="neu-raised rounded-3xl p-6">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                <ClipboardCheck className="w-5 h-5 text-emerald-600" />Approval Status
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[
                  { label: 'Supervisor', approved: trackerData.incident.supervisor_approved, date: trackerData.incident.supervisor_approved_at, comments: trackerData.incident.supervisor_comments },
                  { label: 'HSE Officer', approved: trackerData.incident.hse_approved, date: trackerData.incident.hse_approved_at, comments: trackerData.incident.hse_comments },
                  { label: 'Ops Manager', approved: trackerData.incident.ops_manager_approved, date: trackerData.incident.ops_manager_approved_at, comments: trackerData.incident.ops_manager_comments },
                  { label: 'HR', approved: trackerData.incident.hr_approved, date: trackerData.incident.hr_approved_at, comments: trackerData.incident.hr_comments },
                  { label: 'MD', approved: trackerData.incident.md_approved, date: trackerData.incident.md_approved_at, comments: trackerData.incident.md_comments },
                ].map(approval => (
