...
import { 
  ...
  Search, Download, ChevronRight, Sun, Moon, Sparkles, MapPin,
  Calendar, Timer, BarChart3, User, Mail, Phone, Shield, Briefcase,
  Hash, Building2
} from 'lucide-react'

export default function Timesheets() {
  const { attendanceRecords, timesheetStats, fetchAttendanceRecords, fetchTimesheetStats } = useAttendanceStore()
  ...
  const downloadCSV = () => {
    ...
    csv += `Absent Days,${stats?.absent || 0}\n`
    csv += `Late Days,${stats?.late || 0}\n`
    csv += `Total Hours,${stats?.totalHours || 0}h\n`
    ...
    attendance.forEach(r => {
      csv += [
        r.attendance_date || '',
        r.attendance_date ? new Date(r.attendance_date + 'T00:00:00').toLocaleDateString('en-ZA', { weekday: 'long' }) : '',
        r.clock_in_time ? new Date(r.clock_in_time).toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-',
        r.clock_out_time ? new Date(r.clock_out_time).toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-',
    ...
    a.href = url
    a.download = `Timesheet_${emp.first_name}_${emp.last_name}_${new Date().toISOString().split('T')[0]}.csv`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    window.URL.revokeObjectURL(url)
    toast.success('Report downloaded! Opens in Excel')
  }
