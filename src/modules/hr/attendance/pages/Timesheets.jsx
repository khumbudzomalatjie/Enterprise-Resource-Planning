...
export default function Timesheets() {
  ...
  const searchAndShowEmployee = async (searchTerm) => {
    ...
    // Find employee by name, code, or email
  ...
  const formatDateTime = (d) => d ? new Date(d).toLocaleString('en-ZA', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '-'
    ...
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-slate-500 mt-1">Search an employee to view their complete attendance history</p>
        </motion.div>
        ...
        <div className="neu-raised rounded-2xl p-4 mb-6">
          <form onSubmit={handleSearch} className="flex flex-wrap gap-3 items-end">
            ...
            {showEmployeeDetail && (
              <button type="button" onClick={clearEmployeeSearch} className="neu-raised neu-btn px-4 py-3 rounded-xl bg-slate-600 text-white text-sm">
                Clear
              </button>
            )}
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-4 py-3 neu-inset rounded-xl text-sm">
              <option value="all">All Status</option><option value="present">Present</option><option value="absent">Absent</option><option value="late">Late</option>
            </select>
          </form>
        </div>
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 space-y-4">
              {loadingEmployee ? (
                ...
                <>
                  ...
                  {employeeStats && (
                    <div className="grid grid-cols-3 md:grid-cols-7 gap-3">
                      {[
                        { label: 'Total Records', value: employeeStats.total, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20' },
                        ...
                        { label: 'Total Hours', value: `${employeeStats.totalHours}h`, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/20' },
                  ...
                  <div className="neu-raised rounded-3xl overflow-hidden">
                    <h3 className="text-lg font-semibold p-4 pb-0 text-slate-800 dark:text-white">Attendance History ({employeeAttendance.length} records)</h3>
                    <div className="overflow-x-auto p-4">
                          ...
                          {employeeAttendance.map(r => (
                            <tr key={r.id} className="border-t border-slate-100 dark:border-slate-700/30">
                              ...
                              <td className="py-2 px-3">
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${r.status === 'present' ? 'bg-emerald-100 text-emerald-700' : r.status === 'late' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{r.status}</span>
                              </td>
                              <td className="py-2 px-3 text-xs capitalize">{r.check_in_method?.replace(/_/g, ' ') || '-'}</td>
                              <td className="py-2 px-3 text-xs">
                                {r.check_in_latitude ? <MapPin className="w-3 h-3 text-emerald-500 inline" /> : '-'}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                      ...
                      {employeeAttendance.length === 0 && (
                        <p className="text-center py-8 text-slate-500">No attendance records found</p>
                      )}
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        {employeeShifts.map(s => (
                          <div key={s.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-700/30 text-sm">
                            <div>
                              <span className="font-medium">{s.shift_templates?.shift_name}</span>
                              <span className="text-xs text-slate-500 ml-2">{s.shift_templates?.start_time?.slice(0,5)} - {s.shift_templates?.end_time?.slice(0,5)}</span>
                            </div>
                ...
                <div className="neu-raised rounded-3xl p-8 text-center">
                  ...
                  <p className="text-slate-500">Employee not found. Try searching by name, code, or email.</p>
                </div>
              )}
            </motion.div>
          )}
        ...
        <div className="neu-raised rounded-3xl overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50">
                  ...
                  <th className="text-left py-3 px-3 text-xs text-slate-500">Clock In</th>
                  <th className="text-left py-3 px-3 text-xs text-slate-500">Clock Out</th>
                  <th className="text-left py-3 px-3 text-xs text-slate-500 hidden lg:table-cell">Hours</th>
                  <th className="text-left py-3 px-3 text-xs text-slate-500">Status</th>
                </tr>
              </thead>
              <tbody>
                {attendanceRecords.map((record) => (
                  <tr key={record.id} className="border-t border-slate-100 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                    ...
                    <td className="py-3 px-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${record.status === 'present' ? 'bg-emerald-100 text-emerald-700' : record.status === 'late' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{record.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {attendanceRecords.length === 0 && !showEmployeeDetail && (
            <div className="text-center py-12"><Search className="w-12 h-12 text-slate-300 mx-auto mb-2" /><p className="text-slate-500">Search for an employee to view their timesheet</p></div>
          )}
        </div>
      </main>
    </div>
