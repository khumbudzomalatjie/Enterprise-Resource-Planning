...
export default function Timesheets() {
  ...
  const handleViewDetail = (record) => { setSelectedRecord(record); setShowDetail(true) }
              ...
              <tbody>
                {attendanceRecords.map((record) => (
                  <tr key={record.id} className="border-t border-slate-100 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-700/20 cursor-pointer" onClick={() => handleViewDetail(record)}>
                    ...
                    <td className="py-3 px-3"><span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${record.status === 'present' ? 'bg-emerald-100 text-emerald-700' : record.status === 'late' ? 'bg-amber-100 text-amber-700' : record.status === 'absent' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>{record.status}</span></td>
                    <td className="py-3 px-3 text-right"><button onClick={(e) => { e.stopPropagation(); handleViewDetail(record) }} className="p-1.5 rounded-lg hover:bg-emerald-100 text-slate-400 hover:text-emerald-600"><Eye className="w-4 h-4" /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {attendanceRecords.length === 0 && <div className="text-center py-12"><Clock className="w-12 h-12 text-slate-300 mx-auto mb-2" /><p className="text-slate-500">No timesheet records found</p></div>}
        </div>
        ...
        <AnimatePresence>
          {showDetail && selectedRecord && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowDetail(false)}>
              <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white dark:bg-slate-800 rounded-3xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between mb-4"><h3 className="text-xl font-bold">Timesheet Detail</h3><button onClick={() => setShowDetail(false)} className="p-2 rounded-lg hover:bg-slate-100"><XCircle className="w-5 h-5" /></button></div>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {selectedRecord.check_in_latitude && <div className="col-span-2"><p className="text-xs text-slate-500">GPS</p><p className="text-xs">{selectedRecord.check_in_latitude?.toFixed(6)}, {selectedRecord.check_in_longitude?.toFixed(6)}</p></div>}
                </div>
