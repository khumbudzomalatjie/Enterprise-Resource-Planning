...
export default function LeaveManagement() {
  ...
  const handleApplyLeave = async (e) => {
    ...
    if (!applyForm.start_date || !applyForm.end_date) { toast.error('Please select start and end dates'); return }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">SA BCEA Compliant Leave System</p>
          </div>
          <button onClick={() => setShowApplyForm(!showApplyForm)} className="px-6 py-3 bg-emerald-600 text-white rounded-2xl font-medium flex items-center gap-2 hover:bg-emerald-700 transition-colors shadow-lg">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="neu-raised rounded-3xl p-6 mb-6 border-2 border-emerald-200 dark:border-emerald-800">
            ...
            <form onSubmit={handleApplyLeave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Leave Type Dropdown - NO DAYS DISPLAYED */}
                <div>
                  ...
                  >
                    ...
                    {leaveTypes.map(lt => (
                      <option key={lt.id || lt.code} value={lt.id || lt.code}>
                        {lt.name} {lt.paid ? '(Paid)' : '(Unpaid/UIF)'}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button type="button" onClick={() => setShowApplyForm(false)} className="px-5 py-2.5 bg-slate-200 dark:bg-slate-600 rounded-xl text-sm font-medium hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">Cancel</button>
                <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium flex items-center gap-2 hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-lg">
            ...
            {leaveBalances.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {leaveBalances.slice(0, 8).map(balance => (
                  <div key={balance.id} className="rounded-xl p-4 text-center border-2 transition-all hover:shadow-md bg-white dark:bg-slate-700/30" style={{ borderColor: balance.leave_types?.color || '#10b981' }}>
                    ...
                    <div className="w-full h-2 bg-slate-200 dark:bg-slate-600 rounded-full mt-2 mb-1 overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${balance.allocated_days > 0 ? Math.min((balance.used_days / balance.allocated_days) * 100, 100) : 0}%`, backgroundColor: balance.leave_types?.color || '#10b981' }}></div>
                    </div>
                    <p className="text-2xl font-bold mt-1" style={{ color: balance.leave_types?.color || '#10b981' }}>{balance.remaining_days || 0}</p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500">of {balance.allocated_days} days remaining</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Contact HR to set up your leave balances</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'balances' && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            ...
            {leaveBalances.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30">
                      <th className="text-left py-3 px-4 text-slate-500 dark:text-slate-400">Leave Type</th>
                      <th className="text-right py-3 px-4 text-slate-500 dark:text-slate-400">Allocated</th>
                      <th className="text-right py-3 px-4 text-slate-500 dark:text-slate-400">Used</th>
                      <th className="text-right py-3 px-4 text-slate-500 dark:text-slate-400">Pending</th>
                      <th className="text-right py-3 px-4 text-slate-500 dark:text-slate-400">Remaining</th>
                      <th className="text-left py-3 px-4 text-slate-500 dark:text-slate-400">Cycle</th>
                    </tr>
                  </thead>
                  <tbody>
                    {leaveBalances.map(b => (
                      <tr key={b.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                        <td className="py-3 px-4"><div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: b.leave_types?.color || '#10b981' }}></span><span className="font-medium text-slate-800 dark:text-white">{b.leave_types?.name}</span></div></td>
                        ...
                        <td className="py-3 px-4 text-xs text-slate-500 dark:text-slate-400">{formatDate(b.cycle_start)} - {formatDate(b.cycle_end)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="text-center py-8"><p className="text-slate-500 dark:text-slate-400">No leave balances found</p></div>
            )}
          </div>
        )}
        ...
        {activeTab === 'history' && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-1.5 neu-inset rounded-lg text-sm text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700">
            ...
            {leaveRequests.length > 0 ? (
                  ...
                  <tbody>
                    {leaveRequests.filter(lr => statusFilter === 'all' || lr.status === statusFilter).map(lr => (
                      <tr key={lr.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                        <td className="py-3 px-4"><div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: lr.leave_types?.color || '#10b981' }}></span><span className="text-slate-700 dark:text-slate-300">{lr.leave_types?.name || 'N/A'}</span></div></td>
                        <td className="py-3 px-4 text-xs text-slate-600 dark:text-slate-400">{formatDate(lr.start_date)} - {formatDate(lr.end_date)}</td>
                        <td className="py-3 px-4 text-right font-semibold text-slate-700 dark:text-slate-300">{lr.total_days}</td>
                        <td className="py-3 px-4 text-xs text-slate-500 dark:text-slate-400 max-w-[200px] truncate">{lr.reason || '-'}</td>
                        <td className="py-3 px-4 text-center"><span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(lr.status)}`}>{lr.status}</span></td>
                        <td className="py-3 px-4 text-xs text-slate-500 dark:text-slate-400">{formatDate(lr.submitted_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="text-center py-8"><FileText className="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-2" /><p className="text-slate-500 dark:text-slate-400">No leave requests found</p></div>
            )}
          </div>
        )}
        ...
        {activeTab === 'approvals' && isHR && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-4">Pending Approvals ({pendingApprovals.length})</h2>
            {pendingApprovals.length > 0 ? (
              <div className="space-y-3">
                {pendingApprovals.map(req => (
                  <div key={req.id} className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: req.leave_types?.color || '#10b981' }}></span><span className="font-semibold text-slate-800 dark:text-white">{req.employees?.first_name} {req.employees?.last_name}</span><span className="text-xs text-slate-500 dark:text-slate-400">{req.employees?.employee_code}</span></div>
                        <p className="text-sm text-slate-600 dark:text-slate-400"><strong>{req.leave_types?.name}</strong>: {formatDate(req.start_date)} - {formatDate(req.end_date)} <span className="ml-2 font-semibold">({req.total_days} days)</span></p>
                        {req.reason && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Reason: {req.reason}</p>}
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Submitted: {formatDate(req.submitted_at)}</p>
                      </div>
                      <div className="flex gap-2 flex-shrink-0">
                        <button onClick={() => handleApprove(req.id)} className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 transition-colors flex items-center gap-1"><CheckCircle2 className="w-4 h-4" /> Approve</button>
                        <button onClick={() => handleReject(req.id)} className="px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700 transition-colors flex items-center gap-1"><XCircle className="w-4 h-4" /> Reject</button>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12 text-slate-400 dark:text-slate-500"><CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-3" /><p className="text-lg">No pending leave requests</p><p className="text-sm mt-1">All leave requests have been processed</p></div>
            )}
          </div>
        )}
      </main>
