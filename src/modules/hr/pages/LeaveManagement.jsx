                  ...
                  <tbody>
                    {leaveRequests.filter(lr => statusFilter === 'all' || lr.status === statusFilter).map(lr => (
                      <tr key={lr.id} className="border-b hover:bg-slate-50">
                        ...
                        <td className="py-3 px-4 text-xs truncate max-w-[200px]">{lr.reason || '-'}</td>
                        ...
                        <td className="py-3 px-4 text-xs">{formatDate(lr.submitted_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {activeTab === 'approvals' && isHR && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            ...
            {pendingApprovals.length > 0 ? (
                ...
                {pendingApprovals.map(req => (
                  <div key={req.id} className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 rounded-xl p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: req.leave_types?.color }}></span><span className="font-semibold">{req.employees?.first_name} {req.employees?.last_name}</span><span className="text-xs text-slate-500">{req.employees?.employee_code}</span></div>
                        <p className="text-sm"><strong>{req.leave_types?.name}</strong>: {formatDate(req.start_date)} - {formatDate(req.end_date)} <span className="font-semibold">({req.total_days} days)</span></p>
                        {req.reason && <p className="text-xs text-slate-500 mt-1">Reason: {req.reason}</p>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleApprove(req.id)} className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm"><CheckCircle2 className="w-4 h-4 inline mr-1" />Approve</button>
                        <button onClick={() => handleReject(req.id)} className="px-4 py-2 bg-red-600 text-white rounded-xl text-sm"><XCircle className="w-4 h-4 inline mr-1" />Reject</button>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12 text-slate-400"><CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-3" /><p>No pending requests</p></div>
            )}
          </div>
        )}
      </main>
