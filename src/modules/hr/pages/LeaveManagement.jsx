  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      if (employeeId) {
        ...
        const enrichedMyReqs = (myReqs || []).map(req => ({
          ...req,
      ...
      if (isApprover) {
        ...
        const enrichedPending = await Promise.all((pending || []).map(async (req) => {
          let empInfo = null
          if (req.employee_id) {
            const { data: emp } = await supabase
              .from('employees')
  ...
  return (
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        {loading && <div className="text-center py-12"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-emerald-600 mx-auto mb-3"></div><p>Loading...</p></div>}
                  ...
                  <motion.div key={type.id || type.code} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: type.color + '20' }}><Icon className="w-5 h-5" style={{ color: type.color }} /></div>
                        <div><h3 className="font-bold text-sm">{type.name}</h3><p className="text-[10px] text-slate-400">{type.description}</p></div>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); openLeaveApplication(type.id || type.code) }} className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center hover:bg-emerald-200"><Plus className="w-4 h-4 text-emerald-600" /></button>
                    </div>
                    <div className="w-full h-2.5 bg-slate-200 dark:bg-slate-600 rounded-full mb-3 overflow-hidden"><div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(usagePercent, 100)}%`, backgroundColor: type.color }}></div></div>
        ...
        {activeTab === 'my-leave' && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <h2 className="text-lg font-bold mb-4">My Leave History</h2>
            {myRequests.length > 0 ? (
                  ...
                  <tbody>
                    {myRequests.map(r => (
                      <tr key={r.id} className="border-b hover:bg-slate-50">
                        ...
                        <td className="py-3 px-4 text-xs max-w-[150px] truncate">{r.reason || '-'}</td>
        ...
        {activeTab === 'approvals' && isApprover && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <h2 className="text-lg font-bold mb-4">Pending Approvals ({pendingApprovals.length})</h2>
                  ...
                  <div key={req.id} className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 rounded-xl p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: req.leave_types?.color || '#10b981' }}></span><span className="font-bold">{req.employees?.first_name} {req.employees?.last_name}</span><span className="text-xs text-slate-500">{req.employees?.employee_code}</span></div>
                        <p className="text-sm"><strong>{req.leave_types?.name || req.leave_type_id}</strong>: {formatDate(req.start_date)} - {formatDate(req.end_date)} ({req.total_days} days)</p>
                        {req.reason && <p className="text-xs text-slate-500 mt-1">📝 {req.reason}</p>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleApprove(req.id, `${req.employees?.first_name} ${req.employees?.last_name}`)} className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm hover:bg-emerald-700"><CheckCircle2 className="w-4 h-4 inline mr-1" />Approve</button>
                        <button onClick={() => handleDecline(req.id, `${req.employees?.first_name} ${req.employees?.last_name}`)} className="px-4 py-2 bg-red-600 text-white rounded-xl text-sm hover:bg-red-700"><XCircle className="w-4 h-4 inline mr-1" />Decline</button>
                        <button onClick={() => handleRequestInfo(req.id, `${req.employees?.first_name} ${req.employees?.last_name}`)} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700"><MessageSquare className="w-4 h-4 inline mr-1" />Info</button>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            ) : <div className="text-center py-12"><CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-3" /><p>No pending requests</p></div>}
          </div>
        )}

        {/* ═══════════════════════════════════════════ */}
        {/* ALL REQUESTS TAB - WITH APPROVE/DECLINE BUTTONS */}
        {/* ═══════════════════════════════════════════ */}
        {activeTab === 'all-requests' && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <div className="flex flex-wrap justify-between gap-3 mb-4">
              <h2 className="text-lg font-bold">All Leave Requests ({allRequests.length})</h2>
              <div className="flex gap-2 flex-wrap">
                ...
                <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search employee..." className="px-3 py-1.5 neu-inset rounded-lg text-sm w-40" />
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-slate-50 dark:bg-slate-700/30">
                    <th className="text-left py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Employee</th>
                    <th className="text-left py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Type</th>
                    <th className="text-left py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Dates</th>
                    <th className="text-right py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Days</th>
                    <th className="text-center py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Status</th>
                    <th className="text-left py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Submitted</th>
                    {isApprover && <th className="text-center py-3 px-3 text-slate-500 dark:text-slate-400 font-semibold text-xs uppercase">Actions</th>}
                  </tr>
                </thead>
                <tbody>
                  {allRequests.filter(r => {
                    ...
                    <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                      <td className="py-2 px-3">
                        <p className="font-medium text-xs text-slate-700 dark:text-slate-300">{r.employees?.first_name || 'N/A'} {r.employees?.last_name || ''}</p>
                        <p className="text-[10px] text-slate-400">{r.employees?.employee_code}</p>
                      </td>
                      <td className="py-2 px-3">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: r.leave_types?.color || '#10b981' }}></span>
                          <span className="text-xs">{r.leave_types?.name || r.leave_type_id}</span>
                        </div>
                      </td>
                      <td className="py-2 px-3 text-xs text-slate-600 dark:text-slate-400">{formatDate(r.start_date)} - {formatDate(r.end_date)}</td>
                      <td className="py-2 px-3 text-right text-xs font-semibold">{r.total_days}</td>
                      <td className="py-2 px-3 text-center">
                        <span className={`px-2 py-1 rounded-full text-[10px] font-medium ${getStatusBadge(r.status)}`}>{r.status}</span>
                      </td>
                      <td className="py-2 px-3 text-[10px] text-slate-500">{formatDate(r.submitted_at || r.created_at)}</td>
                      {isApprover && (
                        <td className="py-2 px-3 text-center">
                          {r.status === 'pending' ? (
                            <div className="flex items-center justify-center gap-1">
                              <button 
                                onClick={() => handleApprove(r.id, `${r.employees?.first_name || 'Employee'} ${r.employees?.last_name || ''}`)}
                                className="px-2.5 py-1 bg-emerald-600 text-white rounded-lg text-[11px] font-medium hover:bg-emerald-700 flex items-center gap-1"
                                title="Approve"
                              >
                                <CheckCircle2 className="w-3 h-3" /> Approve
                              </button>
                              <button 
                                onClick={() => handleDecline(r.id, `${r.employees?.first_name || 'Employee'} ${r.employees?.last_name || ''}`)}
                                className="px-2.5 py-1 bg-red-600 text-white rounded-lg text-[11px] font-medium hover:bg-red-700 flex items-center gap-1"
                                title="Decline"
                              >
                            ...
                            <span className="text-[10px] text-red-600 font-medium">✗ Declined</span>
                          ) : (
              ...
              <div className="space-y-2">
                {auditLogs.slice(0, 30).map(log => (
                  <div key={log.id} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50 dark:bg-slate-700/30 text-xs">
                    ...
                    <span className="text-slate-500 flex-1">{log.remarks || 'No remarks'}</span>
