...
import {
  ...
  FileText, AlertCircle, Shield, Upload,
  MessageSquare, History, Users, Download,
  Search, Settings, BarChart3,
  ChevronRight, Heart, GraduationCap, Plane,
  Star, Minus
} from 'lucide-react'
...
export default function LeaveManagement() {
  ...
  }, [applyForm.start_date, applyForm.end_date, applyForm.is_partial])
  ...
  const handleSubmitLeave = async (e) => {
    ...
    try {
      ...
      setApplyForm({ leave_type_id: '', start_date: '', end_date: '', reason: '', document_url: '', is_partial: false })
      loadAllData()
    } catch (error) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">SA BCEA Compliant · Employee Self-Service</p>
          </div>
        ...
        {/* ═══════════════════════════════════════════ */}
        {/* DASHBOARD TAB - Leave Cards with APPLY button */}
        {/* ═══════════════════════════════════════════ */}
              ...
              {leaveTypes.map(type => {
                ...
                return (
                  <motion.div key={type.id || type.code} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    className="neu-raised rounded-2xl p-5 hover:scale-[1.02] transition-all relative overflow-hidden">
                  ...
                  <div key={req.id} className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 rounded-xl p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: req.leave_types?.color }}></span><span className="font-bold">{req.employees?.first_name} {req.employees?.last_name}</span><span className="text-xs text-slate-500">{req.employees?.employee_code}</span></div>
                        <p className="text-sm"><strong>{req.leave_types?.name}</strong>: {formatDate(req.start_date)} - {formatDate(req.end_date)} ({req.total_days} days)</p>
        ...
        {activeTab === 'all-requests' && isApprover && !loading && (
          <div className="neu-raised rounded-3xl p-6">
            <div className="flex justify-between mb-4">
              ...
              <div className="flex gap-2">
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-1.5 neu-inset rounded-lg text-sm"><option value="all">All</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Declined</option></select>
            ...
            <div ref={reportRef} className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b bg-slate-50"><th className="text-left py-3 px-3">Employee</th><th className="text-left py-3 px-3">Type</th><th className="text-left py-3 px-3">Dates</th><th className="text-right py-3 px-3">Days</th><th className="text-center py-3 px-3">Status</th></tr></thead>
                <tbody>
            ...
            {auditLogs.length > 0 ? (
              <div className="space-y-2">
                {auditLogs.slice(0, 30).map(log => (
                  <div key={log.id} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50 text-xs">
                ...
                <form onSubmit={handleSubmitLeave} className="space-y-4">
                  <div>
                    ...
                    <select value={applyForm.leave_type_id} onChange={e => setApplyForm({...applyForm, leave_type_id: e.target.value})} className="w-full p-3 neu-inset rounded-xl mt-1" required>
                      ...
                      {leaveTypes.map(lt => <option key={lt.id || lt.code} value={lt.id || lt.code}>{lt.name} ({lt.days_allowed} days - {lt.paid ? 'Paid' : 'Unpaid/UIF'})</option>)}
                    </select>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <label htmlFor="partial" className="text-sm">Half day leave</label>
                  </div>
                  {calculatedDays > 0 && (
