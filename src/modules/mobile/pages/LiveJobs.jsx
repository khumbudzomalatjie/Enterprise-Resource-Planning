...
export default function LiveJobs() {
  ...
  const searchJob = async (number) => {
    ...
    try {
      // SIMPLE QUERY - Just get the job first
      ...
      if (error || !job) {
        ...
        if (jobs && jobs.length > 0) {
          await loadJobDetails(jobs[0])
          return
        }
        
        toast.error(`Job "${searchNumber}" not found`)
        setSelectedJob(null)
        setLoading(false)
        setSearching(false)
        return
      ...
      toast.error('Error searching for job: ' + error.message)
      setLoading(false)
      setSearching(false)
    }
  }

  // Load all related data for a job
  const loadJobDetails = async (job) => {
    try {
      // Load related data separately to avoid join errors
      ...
      if (assignmentsWithEmployees.length > 0) {
        const employeeIds = assignmentsWithEmployees.map(a => a.employee_id).filter(Boolean)
        if (employeeIds.length > 0) {
          const { data: employees } = await supabase.from('employees').select('*').in('id', employeeIds)
          assignmentsWithEmployees = assignmentsWithEmployees.map(a => ({
            ...a,
      ...
      if (invoicesWithPayments.length > 0) {
        const invoiceIds = invoicesWithPayments.map(inv => inv.id)
        const { data: payments } = await supabase.from('payments').select('*').in('invoice_id', invoiceIds)
      ...
      setSelectedJob({
        ...
        job_categories: categoryRes.data,
        job_assignments: assignmentsWithEmployees,
        job_photos: photosRes.data || [],
        job_task_items: tasksRes.data || [],
        quality_inspections: inspectionsRes.data || [],
        quotations: quotationsRes.data ? [quotationsRes.data] : [],
      ...
      // Still show the job even if related data fails
      setSelectedJob({
        ...job,
        clients: null,
        job_categories: null,
    ...
    <div className={`min-h-screen font-['Inter'] transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="neu-raised rounded-3xl p-6 mb-8">
          ...
          <p className="text-slate-500 mb-6">Enter a Job Number to view the complete job history</p>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-slate-800 rounded-xl shadow-lg border z-50 max-h-[250px] overflow-y-auto">
                  {filteredSuggestions.map(job => (
                    <button key={job.job_number}
                      ...
                      className="w-full text-left px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-between border-b">
                      <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 items-center">
            <p className="text-xs text-slate-400">Available jobs:</p>
            {availableJobs.slice(0, 8).map(job => (
              <button key={job.job_number}
                onClick={() => { setJobNumber(job.job_number); searchJob(job.job_number) }}
                className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-xs font-mono text-blue-600 hover:bg-blue-50">
                {job.job_number}
              </button>
            ))}
          </div>
        ...
        {/* Job Display - simplified for space, full tabs remain the same */}
        {selectedJob && !loading && (
          <div>
            ...
            {activeTab === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  ...
                  { icon: Users, label: 'Cleaners', value: `${selectedJob.job_assignments?.length || 0} assigned` },
                  { icon: CheckCircle2, label: 'Status', value: selectedJob.status?.replace('_', ' ') },
                  { icon: DollarSign, label: 'Amount', value: formatCurrency(selectedJob.quoted_amount) },
            ...
            {/* Other tabs remain the same as previous file - Details, Staff, Timeline, Photos, Inventory, Time, Financials, Customer, Audit */}
            {/* ... (same as previous complete file for all other tabs) ... */}
        ...
        {!selectedJob && !loading && (
          <div className="neu-raised rounded-3xl p-12 text-center">
            ...
            <p className="text-slate-400">Enter a job number or click an available job above</p>
          </div>
        )}
      </main>
    </div>
