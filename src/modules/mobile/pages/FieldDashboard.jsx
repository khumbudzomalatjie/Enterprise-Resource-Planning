...
export default function FieldDashboard() {
  ...
  const loadStats = async () => {
    ...
    const [
      ...
      supabase.from('employees').select('*').eq('employment_status', 'active').eq('department', 'Cleaning'),
      ...
      supabase.from('incident_reports').select('*').eq('status', 'reported'),
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 ml-11">Monitor cleaners, jobs, photos & incidents in real-time</p>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          {[
            ...
            { label: 'Mobile App', icon: Phone, path: '/mobile', color: 'bg-blue-600' },
          ].map(item => (
            <button key={item.label} onClick={() => navigate(item.path)}
              className={`${item.color} text-white rounded-2xl p-3 flex flex-col items-center gap-1 hover:scale-105 transition-transform text-sm font-medium`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {recentPhotos.map(photo => (
                <div key={photo.id} className="relative group cursor-pointer rounded-xl overflow-hidden" onClick={() => window.open(photo.photo_url, '_blank')}>
                  ...
                  <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-medium bg-black/50 text-white capitalize">{photo.photo_type}</span>
                </div>
              ))}
            </div>
          ) : (
