...
export const mobileApi = {
  ...
  async getMyJobs(employeeId) {
    ...
    const { data: jobs } = await supabase
      .from('jobs')
      .select(`id, job_number, title, description, status, priority, scheduled_date, scheduled_start_time, scheduled_end_time, site_address, site_city, site_contact_name, site_contact_phone, access_instructions, special_instructions, notes, cleaners_required, estimated_duration_minutes, quoted_amount, clients(company_name, client_code, phone, city), job_categories(name, color)`)
      .in('id', jobIds)
  ...
  async selectJob(jobId, employeeId) {
    ...
    const { data: existing } = await supabase.from('field_job_assignments').select('id').eq('job_id', jobId).eq('employee_id', employeeId).maybeSingle()
    if (existing) {
      await supabase.from('field_job_assignments').update({ assignment_status: 'assigned', assigned_by: userData.user?.id, assigned_at: new Date().toISOString(), released_at: null, released_by: null, release_reason: null }).eq('id', existing.id)
    } else {
      await supabase.from('field_job_assignments').insert([{ job_id: jobId, employee_id: employeeId, assigned_by: userData.user?.id, assignment_status: 'assigned', assigned_at: new Date().toISOString() }])
    }
  ...
  async startJob(jobId, employeeId, lat, lng) {
    await supabase.from('field_job_assignments').update({ assignment_status: 'in_progress', started_at: new Date().toISOString(), check_in_time: new Date().toISOString(), check_in_latitude: lat, check_in_longitude: lng }).eq('job_id', jobId).eq('employee_id', employeeId)
    await supabase.from('jobs').update({ status: 'in_progress', updated_at: new Date().toISOString() }).eq('id', jobId)
    return { success: true }
  },
  ...
  // ============================================
  // PHOTOS - Upload before/after pictures
  // ============================================
  ...
  async clockIn(employeeId, jobId, lat, lng) {
    ...
    await supabase.from('attendance_records').upsert([{ employee_id: employeeId, attendance_date: new Date().toISOString().split('T')[0], clock_in_time: new Date().toISOString(), check_in_method: 'gps', check_in_latitude: lat, check_in_longitude: lng, status: 'present' }], { onConflict: 'employee_id,attendance_date' })
    return { success: true }
  },
